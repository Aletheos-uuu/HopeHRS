 ...
 * Returns rows ordered by active_employee_count descending (highest department first).
 *
 ...
 * so the timeline reads earliest → latest.
 *
...
export async function getEmployeeFullHistory(empNo) {
  ...
  // Flatten nested join objects for easier consumption by the UI component
  return (data ?? []).map((row) => ({
    empNo:        row.empNo,
    jobCode:      row.jobCode,
