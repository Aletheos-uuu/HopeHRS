...
CREATE TABLE public.department (
  ...
  CONSTRAINT dept_status_ck CHECK (record_status IN ('ACTIVE','INACTIVE'))
);
...
CREATE TABLE public.job (
  ...
  stamp         VARCHAR(60),
  PRIMARY KEY (jobCode),
  CONSTRAINT job_status_ck CHECK (record_status IN ('ACTIVE','INACTIVE'))
);
...
CREATE TABLE public.employee (
  ...
  stamp         VARCHAR(60),
  PRIMARY KEY (empno),
  CONSTRAINT emp_status_ck CHECK (record_status IN ('ACTIVE','INACTIVE'))
);

-- JOBHISTORY
CREATE TABLE public.jobHistory (
  empNo         VARCHAR(5)    NOT NULL REFERENCES public.employee(empno),
  jobCode       VARCHAR(4)    NOT NULL REFERENCES public.job(jobCode),
  ...
  FOREIGN KEY (deptCode) REFERENCES public.department(deptCode),
  CONSTRAINT jh_status_ck CHECK (record_status IN ('ACTIVE','INACTIVE'))
);
...
INSERT INTO public.department (deptCode, deptName) VALUES
  ('ACT','Accounting'),('BR1','Sales Branch 1'),('BR2','Sales Branch 2'),
  ('EXC','Executive'),('HRD','Human Resource'),('IT','Information Tech'),
  ('PAY','Payroll'),('WHS','Warehouse');

INSERT INTO public.job (jobCode, jobDesc) VALUES
  ('PRES','President'),('VP','Vice president'),('MGR','Manager'),
  ('SA1','Sales Agent 1'),('SA2','Sales Agent 2'),('SPVR','Supervisor'),
  ('CLK1','Clerk 1'),('CLK2','Clerk 2'),('PR1','Programmer 1'),
  ('PR2','Programmer 2'),('ANYS','Analyst'),('ACCT','Accountant'),
  ('WMAN','Warehouse man'),('HRS','HR Specialist');

INSERT INTO public.employee (empno,lastname,firstname,gender,birthdate,hiredate,sepDate) VALUES
...
INSERT INTO public.jobHistory (empNo,jobCode,effDate,salary,deptCode) VALUES
