    ...
    >
      ...
      >
        ...
        <div className="text-center space-y-1">
          <p className="text-sm font-semibold text-gray-900">Remove job history entry?</p>
          <p className="text-xs text-gray-500">
            Effective <span className="font-medium text-gray-700">{formatDate(row.effDate)}</span>
            {' · '}
            <span className="font-medium text-gray-700">{row.jobDesc ?? row.jobCode}</span>
          </p>
...
export default function JobHistoryPanel({ empno }) {
  ...
  const [jobs, setJobs]           = useState({})    // jobCode → jobDesc
  ...
  async function fetchAll() {
    ...
    setError(null)
    try {
      const [{ data: jhData, error: jhErr }, { data: jobData, error: jobErr }, { data: deptData, error: deptErr }] =
        await Promise.all([
          supabase
            .from('job_history')
            .select('*')
          ...
          supabase.from('jobs').select('jobCode, jobDesc'),
          supabase.from('departments').select('deptCode, deptName'),
        ])
  ...
  async function handleDelete() {
    ...
    try {
      const { error } = await supabase
        ...
        .eq('id', deleteTarget.id)   // TODO: adjust PK column name if different
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100">
                {['Eff. Date', 'Job', 'Department', 'Salary', 'Stamp'].map((h) => (
                  <th
                    key={h}
      ...
      {deleteTarget && (
        <DeleteConfirm
          row={{ ...deleteTarget, jobDesc: jobs[deleteTarget.jobCode] }}
