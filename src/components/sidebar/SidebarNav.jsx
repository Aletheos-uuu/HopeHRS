...
export default function SidebarNav({ onNavigate }) {
  ...
  // Split into groups: items without a group go under "HR Management",
  // items with group: 'Reports' get their own section header.
  const hrItems = visibleItems.filter((i) => !i.group)
  const reportItems = visibleItems.filter((i) => i.group === 'Reports')
