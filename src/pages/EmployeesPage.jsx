...
function EditIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
      <path d="M11.013 1.427a1.75 1.75 0 0 1 2.474 0l1.086 1.086a1.75 1.75 0 0 1 0 2.474L5.81 13.75a.75.75 0 0 1-.374.206l-3.5.875a.75.75 0 0 1-.911-.912l.875-3.5a.75.75 0 0 1 .206-.373L11.013 1.427ZM12.72 2.488a.25.25 0 0 0-.353 0L3.496 11.359l-.57 2.283 2.283-.571 8.87-8.871a.25.25 0 0 0 0-.353L12.72 2.488Z" fill="currentColor" />
    </svg>
  )
}
...
function TrashIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
      <path d="M11 1.75V3h2.25a.75.75 0 0 1 0 1.5H2.75a.75.75 0 0 1 0-1.5H5V1.75C5 .784 5.784 0 6.75 0h2.5C10.216 0 11 .784 11 1.75ZM4.496 6.675l.66 6.6a.25.25 0 0 0 .249.225h5.19a.25.25 0 0 0 .249-.225l.66-6.6a.75.75 0 0 1 1.492.149l-.66 6.6A1.748 1.748 0 0 1 10.595 15h-5.19a1.75 1.75 0 0 1-1.741-1.576l-.66-6.6a.75.75 0 1 1 1.492-.149ZM6.5 1.75V3h3V1.75a.25.25 0 0 0-.25-.25h-2.5a.25.25 0 0 0-.25.25Z" fill="currentColor" />
    </svg>
  )
}
...
export default function EmployeesPage() {
  ...
  function handleAddSuccess()     { setAddOpen(false) }
  function handleEditSuccess()    { setEditTarget(null) }
  function handleDeleteSuccess()  { setDeleteTarget(null) }
  ...
  return (
    <div className="p-6 space-y-6">
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-xs text-gray-400 uppercase tracking-wide">
              <tr>
                ...
                {showStamp && <th className="px-4 py-3 text-left font-medium">Stamp</th>}
                {(canEdit || canDel) && <th className="px-4 py-3 text-left font-medium">Actions</th>}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {filtered.map((emp) => (
                <tr
                  key={emp.empno}
                  className={emp.status === 'INACTIVE' ? 'opacity-50' : 'hover:bg-gray-50 transition-colors'}
                >
                  ...
                  {showStamp && <td className="px-4 py-3"><StampBadge date={emp.stamp} /></td>}
                  {(canEdit || canDel) && (
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        {canEdit && (
                          <IconButton title="Edit employee" onClick={() => setEditTarget(emp)}>
                            <EditIcon />
                          </IconButton>
                        )}
                        {canDel && (
                          <IconButton title="Delete employee" variant="danger" onClick={() => setDeleteTarget(emp)}>
                            <TrashIcon />
                          </IconButton>
                        )}
                      </div>
      ...
      {/* Modals */}
      {canAdd && (
        <AddEmployeeModal open={addOpen} onClose={() => setAddOpen(false)} onSuccess={handleAddSuccess} />
      )}
      {canEdit && editTarget && (
        <EditEmployeeModal open={!!editTarget} employee={editTarget} onClose={() => setEditTarget(null)} onSuccess={handleEditSuccess} />
      )}
      {canDel && deleteTarget && (
        <SoftDeleteConfirmDialog open={!!deleteTarget} employee={deleteTarget} onClose={() => setDeleteTarget(null)} onSuccess={handleDeleteSuccess} />
      )}
    </div>
  )
}