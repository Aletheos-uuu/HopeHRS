          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                <th className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider px-4 py-3 w-36">Dept Code</th>
                <th className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider px-4 py-3">Department Name</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={colCount} className="p-0">
                    <EmptyState
                      icon={search ? 'search' : 'table'}
                      message={search ? 'No departments match your search.' : 'No departments found.'}
                    />
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={dept.deptcode} className="hover:bg-gray-50/60 transition-colors">
                    ...
                    {showStamp && (
                      <td className="px-4 py-3 text-gray-500 text-xs">
                        {dept.stamp
                          ? <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-violet-100 text-violet-700">{dept.stamp}</span>
                    ...
                    {(canEdit || canDel) && (
                      <td className="px-4 py-3 text-right">
                        <div className="flex justify-end gap-1">
                          {canEdit && <IconButton onClick={() => setEditDept(dept)} title="Edit department"><EditIcon /></IconButton>}
                          {canDel && <IconButton onClick={() => setDeleteDept(dept)} title="Delete department"><TrashIcon /></IconButton>}
                        </div>
                      </td>
                    )}
                  </tr>
